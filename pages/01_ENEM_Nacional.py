...
from dados_compartilhados import carregar_dados, preparar_agregacoes, preparar_tabela_completa
...
with st.spinner("Carregando dados..."):
    df = carregar_dados()
    nac, nac_rede, regiao, uf, rede_ano = preparar_agregacoes(df)
...
cores_regiao = {"Norte": "#2ecc71", "Nordeste": "#e74c3c", "Centro-Oeste": "#f39c12", "Sudeste": "#3498db", "Sul": "#9b59b6"}
...
with k1:
    st.markdown(f"""<div class="kpi-card"><div class="kpi-value">{media_geral:.1f}</div><div class="kpi-label">Média Anual Nacional</div></div>""", unsafe_allow_html=True)
with k2:
    st.markdown(f"""<div class="kpi-card"><div class="kpi-value" style="color:#4ecdc4">+{gap_2014_2025:.1f}</div><div class="kpi-label">Evolução 2014→2025</div></div>""", unsafe_allow_html=True)
with k3:
    st.markdown(f"""<div class="kpi-card"><div class="kpi-value" style="color:#e94560">{gap_priv_pub:.0f}</div><div class="kpi-label">Gap Privada vs Pública</div></div>""", unsafe_allow_html=True)
with k4:
    top_rede = gap_rede.idxmax()
    st.markdown(f"""<div class="kpi-card"><div class="kpi-value" style="color:#ffd93d">{gap_rede.max():.1f}</div><div class="kpi-label">Melhor Rede: {top_rede}</div></div>""", unsafe_allow_html=True)
with k5:
    st.markdown(f"""<div class="kpi-card"><div class="kpi-value">{len(df):,}</div><div class="kpi-label">Escolas-rankings analisados</div></div>""", unsafe_allow_html=True)
...
with c1:
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=nac["ANO"], y=nac["MEDIA"], mode="lines+markers+text",
    ...
    for a in [2014, 2020, 2022, 2025]:
        if a in med_por_ano.index:
            ...
            fig1.add_annotation(x=a, y=y_val, text=f"<b>{y_val:.1f}</b>", showarrow=True, arrowhead=3, arrowsize=1.5, ax=0, ay=-40, font=dict(size=13))
...
with c2:
    ...
    st.markdown(f"""A média nacional subiu **{gap_2014_2025:+.1f} pontos** em 12 anos — um crescimento consistente, com aceleração notável a partir de 2022. O maior salto foi entre {int(max_jump['ANO'])-1} e {int(max_jump['ANO'])} (+{max_jump['DIF']:.1f} pts).""")
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown('<div class="insight-box">', unsafe_allow_html=True)
    st.markdown("**🦠  Pandemia e recuperação**")
    st.markdown("""2020 e 2021 (pandemia) mostraram estabilidade, não queda — surpreendente. A recuperação veio forte em 2022, possivelmente refletindo adaptação ao ensino remoto.""")
    st.markdown("</div>", unsafe_allow_html=True)
...
with c3:
    fig2 = px.line(nac_rede, x="ANO", y="MEDIA", color="REDE", markers=True,
        color_discrete_map=cores_rede, title="Média por Rede de Ensino ao Longo dos Anos")
    fig2.update_traces(line=dict(width=3), marker=dict(size=8))
    fig2.update_layout(height=450, hovermode="x unified", xaxis=dict(dtick=1), margin=dict(l=0, r=0, t=50, b=0))
    st.plotly_chart(fig2, use_container_width=True)

with c4:
    fig_donut = go.Figure(data=[go.Pie(labels=gap_rede.sort_values().index, values=gap_rede.sort_values().values,
    ...
    fig_donut.update_layout(title="Média Geral por Rede", height=350, margin=dict(l=0, r=0, t=50, b=0), showlegend=False)
    st.plotly_chart(fig_donut, use_container_width=True)
    st.markdown('<div class="insight-box">', unsafe_allow_html=True)
    st.markdown("**💰  O abismo particular-público**")
    st.markdown(f"""Escolas privadas (média **{gap_rede['Privada']:.0f}**) superam as estaduais (**{gap_rede['Estadual']:.0f}**) em **{gap_priv_pub:.0f} pontos**. As federais (**{gap_rede['Federal']:.0f}**) são o melhor da rede pública, quase equiparando-se às privadas.""")
    st.markdown("</div>", unsafe_allow_html=True)

st.markdown("---")
st.markdown("## 🌍 Capítulo 2: O Mapa da Desigualdade")
...
with c5:
    fig3 = px.line(regiao, x="ANO", y="MEDIA", color="REGIAO", markers=True,
        color_discrete_map=cores_regiao, title="Média por Região Geográfica")
    fig3.update_traces(line=dict(width=3), marker=dict(size=8))
    fig3.update_layout(height=450, hovermode="x unified", xaxis=dict(dtick=1), margin=dict(l=0, r=0, t=50, b=0))
...
with c6:
    ...
    fig_reg_bar = px.bar(media_reg, x=media_reg.values, y=media_reg.index, orientation="h",
        color=media_reg.index, color_discrete_map=cores_regiao, text=media_reg.round(1).astype(str))
    ...
    st.markdown(f"**🗺️  A diferença regional**")
    st.markdown(f"""O **Sul** lidera com **{media_reg['Sul']:.0f}** pontos, enquanto o **Norte** fica com **{media_reg['Norte']:.0f}**. A diferença de **{diff_sul_norte:.0f} pontos** entre Sul e Norte é maior que o gap entre federal e estadual — um reflexo das desigualdades estruturais do país.""")
    st.markdown("</div>", unsafe_allow_html=True)
...
if len(anos_loc) >= 2:
    ...
    with c7:
        fig_ur = px.line(urb_rural, x="ANO", y="MEDIA", color="LOCALIZACAO", markers=True,
            color_discrete_map={"Urbana": "#3498db", "Rural": "#2ecc71"}, title="Média: Escolas Urbanas vs Rurais")
        fig_ur.update_traces(line=dict(width=3), marker=dict(size=8))
        fig_ur.update_layout(height=400, hovermode="x unified", xaxis=dict(dtick=1), margin=dict(l=0, r=0, t=50, b=0))
    ...
    with c8:
        ...
        st.markdown(f"""Escolas urbanas (**{med_urb:.0f}**) superam as rurais (**{med_rur:.0f}**) em **{gap_ur:.0f} pontos**. O gap urbano-rural é comparável ao gap entre redes federal e municipal.""")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"""Escolas rurais da rede **Federal** (Institutos Federais no interior) atingem **{rural_fed:.0f} pontos**, ante **{urb_priv:.0f}** das urbanas privadas — diferença de **{urb_priv - rural_fed:.0f} pontos**.""")
        st.markdown("</div>", unsafe_allow_html=True)
...
with c9:
    ...
    fig_top_uf = px.bar(top_uf, x=top_uf.values, y=top_uf.index, orientation="h",
        color=top_uf.values, color_continuous_scale="Greens", text=top_uf.round(1).astype(str), title="Top 10 Estados por Média")
    fig_top_uf.update_layout(height=400, yaxis=dict(categoryorder="total ascending"), showlegend=False, margin=dict(l=0, r=0, t=50, b=0))
...
with c10:
    ...
    fig_bot_uf = px.bar(bottom_uf, x=bottom_uf.values, y=bottom_uf.index, orientation="h",
        color=bottom_uf.values, color_continuous_scale="Reds_r", text=bottom_uf.round(1).astype(str), title="5 Estados com Menor Média")
    fig_bot_uf.update_layout(height=400, yaxis=dict(categoryorder="total ascending"), showlegend=False, margin=dict(l=0, r=0, t=50, b=0))
...
top_5_uf = rank_uf[rank_uf["SG_UF_ESC"].isin(rank_uf.groupby("SG_UF_ESC")["RANK"].mean().nsmallest(7).index)]
fig_bump = go.Figure()
for est in sorted(top_5_uf["SG_UF_ESC"].unique()):
    d = top_5_uf[top_5_uf["SG_UF_ESC"] == est].sort_values("ANO")
    fig_bump.add_trace(go.Scatter(x=d["ANO"], y=d["RANK"], mode="lines+markers+text",
        name=est, text=d["RANK"].astype(int), textposition="middle right",
        line=dict(width=4), marker=dict(size=10), hovertemplate="%{x}: %{y:.0f}º lugar<extra></extra>"))
fig_bump.update_layout(title="Ranking dos Melhores Estados (posição 1 = melhor)", height=450,
    yaxis=dict(autorange="reversed", dtick=1, title="Posição"), xaxis=dict(dtick=1), hovermode="x unified", margin=dict(l=0, r=0, t=50, b=0))
...
with c11:
    ...
    for _, row in radar_data.iterrows():
        ...
        fig_radar.add_trace(go.Scatterpolar(r=vals, theta=theta, name=row["DEPENDENCIA"],
            line=dict(width=3, color=cores_rede.get(row["DEPENDENCIA"], "#fff")), marker=dict(size=6)))
    fig_radar.update_layout(title="Perfil de Desempenho por Rede (Radar)", height=450,
        polar=dict(radialaxis=dict(visible=True, range=[450, 700])), margin=dict(l=80, r=80, t=50, b=0))
...
with c12:
    ...
    fig_var = px.bar(maior_var, x=maior_var.values, y=[labels_disc[d] for d in maior_var.index],
        orientation="h", color=maior_var.values, color_continuous_scale="Viridis", title="Disciplinas com Maior Variação entre Redes")
    fig_var.update_layout(height=350, showlegend=False, yaxis_title="", xaxis_title="Desvio Padrão", margin=dict(l=0, r=0, t=50, b=0))
    st.plotly_chart(fig_var, use_container_width=True)
    st.markdown('<div class="insight-box">', unsafe_allow_html=True)
    st.markdown("**✍️  Redação: o grande divisor**")
    st.markdown("""Redação (**RD**) é a disciplina com maior dispersão entre as redes — é onde a desigualdade educacional mais aparece. Escolas privadas investem pesado em treino de redação; públicas, nem sempre.""")
    st.markdown("</div>", unsafe_allow_html=True)
...
df_scatter = df[df["ANO"] == ano_scatter].groupby(["DEPENDENCIA", "NO_MUNICIPIO_ESC", "SG_UF_ESC"]).agg(
    MT=("MT", "mean"), LC=("LC", "mean"), MEDIA=("MEDIA", "mean"), ESCOLAS=("MEDIA", "count")).reset_index()
fig_scatter = px.scatter(df_scatter, x="LC", y="MT", color="DEPENDENCIA", size="ESCOLAS",
    color_discrete_map=cores_rede, hover_name="NO_MUNICIPIO_ESC",
    hover_data={"SG_UF_ESC": True, "MEDIA": ":.1f", "ESCOLAS": True},
...
with c13:
    ...
    fig_gap1.add_trace(go.Scatter(x=gap_ano["ANO"], y=gap_ano["GAP_PRIV_PUB"], mode="lines+markers",
        name="Privada − Estadual", line=dict(color="#e94560", width=3), fill="tozeroy", fillcolor="rgba(233,69,96,0.15)"))
    fig_gap1.add_trace(go.Scatter(x=gap_ano["ANO"], y=gap_ano["GAP_FED_EST"], mode="lines+markers",
        name="Federal − Estadual", line=dict(color="#4ecdc4", width=3), fill="tozeroy", fillcolor="rgba(78,205,196,0.15)"))
    fig_gap1.update_layout(title="Evolução do Gap entre Redes", height=400, xaxis=dict(dtick=1), hovermode="x unified", margin=dict(l=0, r=0, t=50, b=0))
...
with c14:
    ...
    for reg in ["Norte", "Nordeste", "Centro-Oeste", "Sudeste", "Sul"]:
        fig_gap2.add_trace(go.Scatter(x=gap_reg.index, y=gap_reg[reg] - gap_reg["Sul"],
            mode="lines+markers", name=f"{reg} − Sul", line=dict(width=2, color=cores_regiao.get(reg))))
    fig_gap2.add_hline(y=0, line_dash="dash", line_color="white", opacity=0.3)
    fig_gap2.update_layout(title="Diferença em relação à Região Sul (referência)", height=400, xaxis=dict(dtick=1), hovermode="x unified", margin=dict(l=0, r=0, t=50, b=0))
...
st.markdown(f"""Ao contrário do que se poderia esperar, o gap entre redes privada e pública **não diminuiu** em 12 anos — variou de {gap_min:.0f} a {gap_max:.0f} pontos (em 2025: {gap_priv_pub_anos.loc[2025, 'GAP']:.0f} pts). As federais, porém, reduziram a distância para as privadas de {gap_fed_2014:.0f} para {gap_fed_2025:.0f} pontos.
Já o abismo regional **aumentou**: Norte e Nordeste perderam terreno relativo para o Sul na segunda metade da década.""")
st.markdown("</div>", unsafe_allow_html=True)
...
with c15:
    ...
    fig_top10 = px.bar(top10, x="MEDIA", y="MUN", orientation="h",
        color="MEDIA", color_continuous_scale="Greens", text=top10["MEDIA"].round(1).astype(str))
    fig_top10.update_layout(title="Top 10 Municípios (média 2014-2025)", height=400,
        yaxis=dict(categoryorder="total ascending"), showlegend=False, margin=dict(l=0, r=0, t=50, b=0))
...
with c16:
    ...
    fig_top_fed = px.bar(top_fed, x="MEDIA", y="MUN", orientation="h",
        color="MEDIA", color_continuous_scale="Reds", text=top_fed["MEDIA"].round(1).astype(str))
...
for idx in evol_pivot.index:
    ...
    valid = row.dropna()
    if len(valid) >= 2:
        first_ano, last_ano = int(valid.index[0]), int(valid.index[-1])
        evol_list.append({"NO_MUNICIPIO_ESC": idx[0], "SG_UF_ESC": idx[1],
            "PRIMEIRO_ANO": first_ano, "ULTIMO_ANO": last_ano,
            "MEDIA_INI": valid.iloc[0], "MEDIA_FIM": valid.iloc[-1], "EVOLUCAO": valid.iloc[-1] - valid.iloc[0]})
...
fig_evol = px.bar(top_evol, x="EVOLUCAO", y="MUN", orientation="h",
    color="EVOLUCAO", color_continuous_scale="Blues", text=top_evol["ROTULO"])
...
with c17:
    ...
    fig_trend.add_trace(go.Scatter(x=nac["ANO"], y=nac["MEDIA"], mode="markers",
        marker=dict(size=10, color="#4ecdc4"), name="Real", hovertemplate="%{x}: %{y:.1f}<extra></extra>"))
    fig_trend.add_trace(go.Scatter(x=x_futuro, y=y_futuro, mode="lines",
        line=dict(color="#e94560", width=3, dash="dash"), name="Tendência Linear"))
    fig_trend.add_trace(go.Scatter(x=x_futuro[x_futuro > 2025], y=y_futuro[x_futuro > 2025],
        mode="lines+markers", line=dict(color="#e94560", width=3, dash="dot"),
        marker=dict(symbol="star", size=12, color="#ffd93d"), name="Projeção"))
    fig_trend.update_layout(title="Projeção da Média Nacional (regressão linear)", height=450,
        xaxis=dict(dtick=1), hovermode="x unified", margin=dict(l=0, r=0, t=50, b=0))
    for a in [2028, 2030]:
        y_val = trend(a)
        fig_trend.add_annotation(x=a, y=y_val, text=f"<b>{y_val:.0f}</b>", showarrow=True, arrowhead=3, ax=0, ay=-30, font=dict(size=11, color="#ffd93d"))
...
with c18:
    ...
    st.markdown(f"""Se a tendência se mantiver, o Brasil pode atingir **{target_2030:.0f} pontos** em 2030. Isso representa um avanço de **{needed:.0f} pontos** em 5 anos — plausível, mas exigirá investimento consistente.""")
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown(f'<div class="insight-box">', unsafe_allow_html=True)
    st.markdown(f"**⚡  O que pode acelerar?**")
    st.markdown("""• Expansão dos Institutos Federais (rede que mais se aproxima da privada)
...
<div style="text-align:center; color:#666; font-size:0.85rem; padding:1rem;">
    ...
    Mais de {len(df):,} registros analisados. Fonte: https://www.gov.br/inep/pt-br/acesso-a-informacao/dados-abertos/microdados/enem<br>
